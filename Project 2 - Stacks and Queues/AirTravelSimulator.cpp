...
{
	// Get max capacity from user
	int userSize;
	cout << "What is the airplanes max capacity?" << endl;
	cin >> userSize;
	m_airport.SetMaxCapacity(userSize);
...
bool AirTravelSimulator::LineUp()
{
	...
	Traveller* passenger = m_pplManager.GetTraveller(randIndex);	

	m_airport.LineUp(passenger);

	m_ptrTravellers.PushBack(passenger);

	passenger->state = LINEUP;

	DisplayMessage(passenger, LINEUP);
...
{
	Traveller* passenger = m_airport.NextInLine();

	m_airplane.Board(passenger);

	passenger->state = BOARDING;

	DisplayMessage(passenger, BOARDING);
...
/**
    1. Create a Traveller* pointer and get the next item from the m_airplane with the Disembark() function.
...
{
	Traveller* passenger = m_airplane.Disembark();

	passenger->state = FINISHED;

	DisplayMessage(passenger, DISEMBARKING);
